...
Original file is located at
    https://colab.research.google.com/drive/1Io7cziw_GVDxGW-wAo6e4ZW2Vl7v_D7X
...
class Model:
  ...
  def _best_split(self, X:np.ndarray, y:np.ndarray):
    ...
    #count of each class in the current
  ...
  def grow_tree(self, X:np.ndarray, y:np.ndarray, depth = 0):
    num_samples_per_class = [np.sum(y==i) for i in range(self.num_classes)]
    predicted_class = np.argmax(num_samples_per_class)
    gini = 1.0 - sum((n/y.size)**2 for n in num_samples_per_class)
    '''
    node = Node(
        gini = self._gini(y),
        num_samples = y.size,
    ...
    '''
    node = Node(gini, y.size, num_samples_per_class, predicted_class)

    #split
    if depth < self.max_depth:
      idx, thr = self._best_split(X,y) #func
      if idx is not None:
